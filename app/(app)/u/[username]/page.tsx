            ...
            {posts.map((p) => {
              ...
              return (
                <Card key={p.id} className="p-3">
                  <Link href={`/p/${p.id}/${p.slug}`} className="block font-medium hover:underline">
                    {p.title}
                  </Link>
                  <div className="mt-1 text-xs text-muted-foreground">
                    {sf ? (
                      <>
                        en <Link href={`/s/${sf.slug}`} className="hover:underline">s/{sf.slug}</Link> ·{" "}
                      </>
                    ) : null}
          ...
          <div className="space-y-2">
            {comments.map((c) => (
              <Card key={c.id} className="p-3 text-sm">
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  {c.score} pts · {formatRelativeEs(c.created_at)} ·{" "}
                  <Link className="hover:underline" href={`/p/${c.post_id}/`}>
                    ver hilo
                  </Link>
                </div>
              </Card>
